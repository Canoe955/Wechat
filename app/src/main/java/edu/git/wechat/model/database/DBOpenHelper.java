...
public class DBOpenHelper extends SQLiteOpenHelper {
    ...
    public void insert(String name , String account , String password){
        db.execSQL("INSERT INTO accounts (name,account,password) VALUES(?,?,?)",new Object[]{name,account,password});
    }

    //删除
    public void delete(String account,String password){
        db.execSQL("DELETE FROM accounts WHERE account = AND password ="+account+password);
    }
    //更新
    public void update(String password){
        db.execSQL("UPDATE accounts SET password = ?",new Object[]{password});
    }

    /**
     * 下面重点说一下查询表accounts全部内容的方法
    ...
    public ArrayList<Accounts> getAccount(){
        ...
        @SuppressLint("Recycle")Cursor cursor =  db.query("accounts",null,null,null,null,null,"name DESC");
        while (cursor.moveToNext()){
            String name = cursor.getString(cursor.getColumnIndex("name"));
            String account = cursor.getString(cursor.getColumnIndex("account"));
            String password = cursor.getString(cursor.getColumnIndex("password"));
            accounts.add(new Accounts(name,account,password));
        }
        return accounts;
    }
